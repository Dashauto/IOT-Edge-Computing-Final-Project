...
static const CLI_Command_Definition_t xVersion = {"version", "version: get firmware version\r\n", (const pdCOMMAND_LINE_CALLBACK)firmware_get_version, 0};
static const CLI_Command_Definition_t xTicks = {"ticks", "ticks: number of ticks since the scheduler was started\r\n", (const pdCOMMAND_LINE_CALLBACK)get_tick, 0};
...
static void FreeRTOS_read(char *character)
{
    // if this function returns -1, it means that no character was received
    int status = SerialConsoleReadCharacter((uint8_t *)character);

    while (status == -1) {
        // block the thread unless we received a character
        xSemaphoreTake(cliCharReadySemaphore, portMAX_DELAY);

        status = SerialConsoleReadCharacter((uint8_t *)character);
    }
}
...
/**
 * @brief    Get the number of ticks since the scheduler was started
*/
...
{
    snprintf(bufCli, CLI_MSG_LEN - 1, "ticks: %d\r\n", xTaskGetTickCount());
    SerialConsoleWriteString(bufCli);
    return pdFALSE;
}
